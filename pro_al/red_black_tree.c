/*************************************************************************
	...
	> Author: ls
	> Mail: 
	> Created Time: Thu 31 Dec 2020 03:07:25 PM CST
 ************************************************************************/
...
Node *insert_maintain(Node *root) {
    ...
    int flag = 0;
    if (root->lchild->color == 0 && root->rchild->color == 0) goto insert_end;//lazy...
...
Node *__insert(Node *root, int key) {
    if (root == NIL) return getNewNde(key);
...
Node *erase_maintain(Node *root) {
    ...
    if (root->lchild->color == 2) {
        if (root->rchild->rchild->color != 0) {//1 2(sharing NIL)
            root->rchild->color = 0;
            root->rchild = right_rotate(root->rchild);
            root->rchild->color = 1;
        }
